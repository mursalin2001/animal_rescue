<div class="flex flex-col m-4 p-4">
    <div class="bg-white shadow-md  rounded-3xl p-4">
        <div class="flex-none">
            <div class="h-96  mb-3 filter" wire:offline.class="grayscale">
                @foreach ($post->postImages as $media)
                    @if ($media->is_image && preg_match('/^.*\.(png|jpg|gif)$/i', $media->path))
                        <img src="{{ url('/storage/' . $media->path) }}" alt="Social"
                            class="w-full h-96 object-scale-down md:object-cover lg:object-cover rounded-2xl"
                    ...
                    @elseif(!$media->is_image && preg_match('/^.*\.(mp4|3gp)$/i', $media->path))
                        <div class="container">
                            <video controls crossorigin playsinline oncontextmenu="return false;"
                                controlsList="nodownload" class="rounded-lg filter" id="player_{{ $post->id }}">
                                <!-- Video files -->
            ...
            <div class="flex-auto ml-3 justify-evenly py-2" wire:offline.class="text-gray-400">
                @can('delete', $post)
                    <button id="delete_{{ $post->id }}" wire:click="showDeletePostModal({{ $post->id }})"
                        ...
                        wire:offline.class.remove="text-red-600" wire:offline.class="text-gray-400" aria-label="Delete"
                        wire:loading.class.remove="text-red-600" wire:loading.class="bg-gray text-gray-400"
                ...
                <div class="flex flex-wrap ">
                    ...
                    <div class="w-full flex-none mb-2 text-xs text-blue-700 font-medium"
                        wire:offline.class.remove="text-blue-700" wire:offline.class="text-gray-400">
                ...
                <div class="flex py-4  text-sm text-gray-600">
                    <div class="flex-1 inline-flex items-center">
                        ...
                        <p class="">{{ !empty($post->upazila) ? $post->upazila : 'Unknown' }},
                            {{-- {{ $post->exact_location }}</p> --}}
                    </div>
                    <div class="flex-1 inline-flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
