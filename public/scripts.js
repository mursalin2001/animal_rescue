...
document.addEventListener('DOMContentLoaded', () => {
    const input = document.getElementById('upazila');
    let upazilaList = [];
    ...
    fetch('/upazila.json')
        .then(res => res.json())
        .then(data => {
            upazilaList = data.upazilas.map(u => u.name);
        });

    input.addEventListener('input', () => {
        const query = input.value.toLowerCase();
        const matches = upazilaList.filter(name => name.toLowerCase().startsWith(query));
        ...
        list.className = 'absolute bg-white border border-gray-300 w-full rounded-b shadow-md z-50';

        matches.slice(0, 5).forEach(name => {
            const item = document.createElement('div');
            item.textContent = name;
            item.className = 'cursor-pointer px-4 py-2 hover:bg-gray-200';
            item.addEventListener('click', () => {
                input.value = name;
                input.dispatchEvent(new Event('change', { bubbles: true })); // Fixes Livewire reversion
                closeSuggestions();
            });
